...
function getInfo(button) {
    ...
    fetch(`http://127.0.0.1:5500/nacho/patients/${patientId}/medRecords`)
    .then(response => {
        if (!response.ok) {
...
function deleteApp(button) {
    ...
    fetch(`http://127.0.0.1:5500/nacho/patients/medRecords/${appId}`, {
        method: 'DELETE',
        headers: {
...
function updateMedRecord(event) {
    ...
    fetch('http://127.0.0.1:5500/nacho/patients/medRecords', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
...
function fillDoctorCombo(combo, callback) {
    ...
    fetch('http://127.0.0.1:5500/nacho/doctors')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
